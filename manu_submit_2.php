        ...
        if (in_array($file_ext,$allowed)) {
            ...
            if ($file_error === 0) {
                if ($file_size <= 5000000) {
                        ...
                        $file_destination =  'uploadAbstract/' . $file_name_new;
                        // echo $file_destination;

                        $root = getcwd();
                        if (move_uploaded_file($file_tmp, $file_destination)) {
                            $sql = "UPDATE files_main SET abstract_name = '" . $file_name_new . "' WHERE ID = " . $last_inserted_id;
                                if (mysqli_query($conn, $sql)) {
                                    echo "Abstract Uploaded !" . $file_name_new;
